...
/** Teinte chaude de la palette : le gris neutre jurerait sur le papier #fefefa. */
const WARM = [4, 1, -3] as const
...
function drawScene(s: Scene) {
  ...
  ctx.fillStyle = `rgb(${Math.round(shade(base + WARM[0], root.r) * 0.92)},${Math.round(
    shade(base + WARM[1], root.g) * 0.92,
  )},${Math.round(shade(base + WARM[2], root.b) * 0.92)})`
...
export function GridReveal({
  ...
  return (
    <div
      ...
      className={cn('relative w-full overflow-hidden rounded-lg bg-surface-2', className)}
